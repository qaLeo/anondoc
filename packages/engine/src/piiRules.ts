...
const INN_10 = /(?<![\d.\-])\d{10}(?![\d.\-])/g
const INN_KEYWORDS = ['инн', 'идентификационный номер', 'inn']
...
// Полис ОМС: ровно 16 цифр подряд + контекст
const OMS = /\b\d{16}\b/g
const OMS_KEYWORDS = ['омс', 'полис', 'oms', 'медицинск', 'страховани']
...
export function detectPii(text: string): RuleResult[] {
  ...
  // ОМС полис (16 цифр подряд) — только с контекстом; проверяем до КАРТА
  for (const m of findAll(text, OMS)) {
    if (hasKeywordNearby(text, m.index, m.index + m[0].length, OMS_KEYWORDS)) {
      results.push({ category: 'ОМС', original: m[0], start: m.index, end: m.index + m[0].length })
    }
  }
